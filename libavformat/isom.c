...
int ff_mp4_read_dec_config_descr(AVFormatContext *fc, AVStream *st, AVIOContext *pb)
{
	...
	if((st->codec->codec_type == AVMEDIA_TYPE_VIDEO) && ((codec_id >= AV_CODEC_ID_MP2) && (codec_id <= AV_CODEC_ID_ILBC) )) {
			av_log(NULL, AV_LOG_WARNING, "[ff_mp4_read_dec_config_descr] Video stream but has strange Audio Codec[0x%x]\n", codec_id);
	} else {
			st->codec->codec_id= codec_id;
	}

    av_dlog(fc, "esds object type id 0x%02x\n", object_type_id);
    av_log(fc, AV_LOG_WARNING, "esds object type id 0x%02x\n", object_type_id);
    len = ff_mp4_read_descr(fc, pb, &tag);
    if (tag == MP4DecSpecificDescrTag) {
        av_dlog(fc, "Specific MPEG4 header len=%d\n", len);
	av_log(fc, AV_LOG_WARNING, "Specific MPEG4 header len=%d\n", len);
        ...
        } else if (st->codec->codec_id == AV_CODEC_ID_MPEG4) {
        	int i;
		int err = 1;
		unsigned long ext_data = 0;
		int size = st->codec->extradata_size;

		for(i=0; i < size -4; i++) {
			ext_data = *((st->codec->extradata)+i) <<24 |*((st->codec->extradata)+ i +1 ) << 16 | *((st->codec->extradata)+i+2) << 8 | *((st->codec->extradata)+i+3);
			if(ext_data == 0x00000120) {
				av_log (fc, AV_LOG_WARNING, "Found the VOL Marker in DCI Info, It is MPEG-4 Content\n");
				err = 0;
				break;
			}
		}        
		...
		if(err) {
			av_log (fc, AV_LOG_WARNING, "Not Found the VOL Marker in DCI Info, Modifying the CAPS to H263\n");
			st->codec->codec_id = AV_CODEC_ID_H263;
			return;
